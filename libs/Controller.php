...
class Controller {
    ...
    public function loadModel($name) {
        $path = 'model/' . $name . '_model.php';							// *** uncomment this line if uploaded (live) *** //
    ...
    public function loadFormModel($name) {
        $path = 'model/tax_return/' . $name . '_Model.php';				// *** uncomment this line if uploaded (live) *** //
    ...
    public function menu() {
        ...
        $trigger = 'tax_return';
        if ($ty['type'] == 'prior' || $ty['type'] == 'Upgrade')
            $trigger = 'tax_returnPY';
    ...
    static function setTrialBalance($coaid, $debit, $credit, $transid, $transdate, $type, $diff) {
        ...
        if ($diff == 'dc') {
            ...
            $tb->balance = $credit - $debit;
        }
//        $tb->transDate = $transdate;

        $tbid = DAOFactory::getTblTrialBalanceDAO()->insert($tb);
        $tbtrans = new TblTrialBalanceTran();
        $tbtrans->trialBalanceId = $tbid;
        $tbtrans->refNo = $transid;
        $tbtrans->typeOfTransaction = $type;
        $tbtrans->date = $transdate;

        DAOFactory::getTblTrialBalanceTransDAO()->insert($tbtrans);
    }
    
    static function copyReverseTB($refnumber,$newRefnumber, $date, $transactiontype){
        $tbs = TblTrialBalanceMySqlExtDAO::getTbTrans(Session::getSession('medorgid'), $refnumber, $transactiontype);
//        print_r($tbs);exit;
        if($tbs){
            foreach ($tbs as $tb){
                
//                echo $tb->coaid;
                Controller::setTrialBalance($tb->coaid, $tb->credit, $tb->debit, $newRefnumber, date('Y-m-d', strtotime($date)), $transactiontype, 'dc');
            }
        }
    }
    
    static function getTaxableYears(){
        $tyear =  DAOFactory::getTblTaxablePeriodDAO()->queryByOrgId(Session::getSession('medorgid'));
        
        if($tyear ){
            $array = array();
            foreach ($tyear as $item){
                $intmonth = (int)date('m',  strtotime($item->month)) - 1;
                $array[] = "'".$item->year .'-'. $intmonth."'" ;
            }
