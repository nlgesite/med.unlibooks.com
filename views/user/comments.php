...
<script>
    $(function() {
        ...
        $('#formtask').submit(function() {
            ...
            $.post("<?= URL ?>invoice/checkDuplicate", {checkitem: 'task', text: $('input[name="taskCode"]').val()},
            function(result) { 
                checkresult = result;
            });

            if (checkresult >= 1) {
                alert('Service ID no. is already exiting');
                return false;
            }
...
<div class="invoiceHolderCNTs">
    <form method="post" action="<?php echo URL ?>invoice/services" class="cnt-formCNTs boxshadow" id="formtask">
        ...
        <div>
            <table style="margin-left: 27px;">	
                ...
                <tr>
                    <td class="taskACNTs">Service ID No.</td>
                    <td><input type="text" class="tAccountCNTs" name="taskCode" value="<?php echo $tbltask->taskCode ?>" readonly></td>
                </tr>
                <tr>
                    <td><div class="descTCNTs">Description:</div></td>
                    <td><input class="itemICNTs" name="description"  id="description" value="<?php echo $tbltask->description ?>" readonly></td>
                </tr>
				<?php
				}
				?>
				...
				<tr>	
                    ...
                    <td><select class="rph" style="border: solid 1px #C8C8C8;" name="glPosting2" id="glPosting2" disabled><option></option>
                         <?php
                            ...
                            foreach ($coas as $coa) {
                                if($coa->accountNum=='4000-001' && $task=='addtask'){ $glid = $coa->id; }
                                ?>
                ...
                <tr>	
                    <td><div style="margin-top:7px;"><input type="checkbox" checked class="taskCheckNew"></div></td>
                </tr>
            </table>
        ...
        <div class="divCNTsBelow">	
            <input type="submit" value="Save" class="saveBCNTs addsavebutton">
